...
def pm_color(value, pollutant='pm10'):
    if pollutant == 'pm10':
        if value <= 30: return "#4fc3f7"    # 좋음 (파란)
        elif value <= 80: return "#81c784"  # 보통 (초록)
        elif value <= 150: return "#ffd54f" # 나쁨 (노랑)
        elif value <= 600: return "#e57373" # 매우나쁨 (빨강)
        else: return "#b71c1c"               # 위험 (진빨강)
    else:
        if value <= 15: return "#4fc3f7"
        elif value <= 35: return "#81c784"
        elif value <= 75: return "#ffd54f"
        elif value <= 500: return "#e57373"
        else: return "#b71c1c"
...
with col1:
    st.markdown("#### 현재 공기질 & 날씨")
    pm10_col = pm_color(input_pm10, 'pm10')
    pm25_col = pm_color(input_pm25, 'pm2.5')
    st.markdown(
        f"<div style='background:{pm10_col}; color:white; width:100px; height:100px; display:flex; "
        f"align-items:center; justify-content:center; font-size:18px; border:none; border-radius:8px; "
        f"margin-bottom:8px;'>PM10<br>{input_pm10}</div>",
        unsafe_allow_html=True
    )
    st.markdown(
        f"<div style='background:{pm25_col}; color:white; width:100px; height:100px; display:flex; "
        f"align-items:center; justify-content:center; font-size:18px; border:none; border-radius:8px;'>PM2.5<br>{input_pm25}</div>",
        unsafe_allow_html=True
    )
    # 기온 표시: 연한 회색 배경+검은 글씨, 기준치 벗어나면 짙은 회색+흰 글씨
    temp_color = 'white' if (input_temp < 0 or input_temp > 28) else 'black'
    bg_color = '#b0bec5' if (input_temp < 0 or input_temp > 28) else '#424242'
    st.markdown(
        f"<div style='background:{bg_color}; color:{temp_color}; width:100px; height:100px; "
        f"display:flex; align-items:center; justify-content:center; font-size:18px; border:none; "
        f"border-radius:8px;'>기온(°C)<br>{input_temp:.1f}</div>",
        unsafe_allow_html=True
    )
